...
async def run_single_cycle():
    ...
    # مزامنة الردود الحية من الشبكة لمنع تكرار أي شخص علّق له السكربت الثاني
    for relay in GLOBAL_RELAYS[:3]:
        try:
            async with websockets.connect(relay, ping_interval=5, ping_timeout=5) as ws:
                req = json.dumps(["REQ", "sync_history", {"authors": [bot_hex], "kinds": [1], "limit": 200}])
                await ws.send(req)
                for _ in range(25):
                    resp = await asyncio.wait_for(ws.recv(), timeout=1.5)
                    data = json.loads(resp)
                    if data[0] == "EVENT" and len(data) >= 3:
                        ...
                        for tag in ev.get("tags", []):
                            if len(tag) >= 2:
                                if tag[0] == "p": GLOBAL_SEEN_SENDERS.add(tag[1].lower())
                                elif tag[0] == "e": GLOBAL_REPLIED_EVENTS.add(tag[1].lower())
                    elif data[0] == "EOSE":
                        break
        except Exception:
            continue
